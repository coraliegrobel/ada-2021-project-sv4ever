...
def proba_lemm_pos(name_file):
	...
	for phrase in sentences:
		...
		for i_mot in range(len(phrase)):
			...
			if phrase[i_mot][0] in pos_entier:
					pos_entier[phrase[i_mot][0]] += 1
			else:
				pos_entier[phrase[i_mot][0]] = 1
		
			# All word of this file.
			if phrase[i_mot][num_lemm] in lemm_entier:
			...
			if str_motif_lemm in motif_lemm:
			...
			if value_uncertainty == 1:
				# Lemm.
				if phrase[i_mot][num_lemm] in lemm_su:
					lemm_su[phrase[i_mot][num_lemm]] += 1
				else:
					lemm_su[phrase[i_mot][num_lemm]] = 1
				# Motif PoS
				if str_motif in motif_su:
					motif_su[str_motif] += 1
				else:
					motif_su[str_motif] = 1
				# Motif lemms.
				if str_motif_lemm in motif_lemm_su:
			...
			if phrase[i_mot][1] == "B" or phrase[i_mot][1] == "I":
				...
				if phrase[i_mot][num_lemm] in lemm_use:
					lemm_use[phrase[i_mot][num_lemm]] += 1
				else:
					lemm_use[phrase[i_mot][num_lemm]] = 1
				# Only pos tag use as uncertainty cue.
				if phrase[i_mot][0] in pos_use:
					pos_use[phrase[i_mot][0]] += 1
				else:
					pos_use[phrase[i_mot][0]] = 1
				# Motif PoS
				if str_motif in motif:
					motif[str_motif] += 1
				else:
					motif[str_motif] = 1
				# Motif Lemms.
				if str_motif_lemm in motif_lemm_use:
				...
				if i_mot + 2 < len(phrase) and phrase[i_mot+1][1] != "O" and phrase[i_mot+2][1] != "O":
					lemm_trigram = phrase[i_mot][2]+"_"+phrase[i_mot+1][2]+"_"+phrase[i_mot+2][2]
					if lemm_trigram in trigram_use:
